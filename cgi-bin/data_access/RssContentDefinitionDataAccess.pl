...
sub execute_query {
	...
	my $dbh = DBI->connect("dbi:Pg:dbname=${\DATABASE};host=${\HOST}", USER, PASSWORD, {AutoCommit=>1, RaiseError=>1, PrintError=>0});
	...
	while(my @row = $sth->fetchrow_array()) {
		...
		push(@atomContentDefinitions, AtomContentDefinition->new(name=>$name, url=>$url));
	}
...
sub get_all_atom_content_definitions {
	return execute_query(' SELECT "url", "name" FROM "AtomcontentDefinitions" WHERE enabled = true ');
#	my @params = (1, 2);
...
sub get_atom_content_definitions_by_name {
	my $name = shift;
	return execute_query(' SELECT "url", "name" FROM "AtomcontentDefinitions" WHERE enabled = true AND name = $1 ', \($name));
}
